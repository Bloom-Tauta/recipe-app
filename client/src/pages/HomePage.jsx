...
function HomePage() {
	...
	useEffect(() => {
		fetch("http://localhost:3001/meals?_limit=50")
			.then((res) => res.json())
			.then((response) => {
			...
			<div className="relative">
				...
				<div className="text-center absolute inset-0 font-bold text-3xl text-white flex flex-col justify-center items-center">
					...
					<p className="p-2 text-3xl">
						Thousands of recipe. Endless inspiration.<br></br>
						Discover our recipe suggestions and get inspired to
						create your own dishes.
					</p>
					<NavLink
						to="/signup"
						className="border bg-orange-600 rounded-md py-2 px-6 my-5">
						TRY IT FOR FREE
					</NavLink>
				</div>
			</div>

			<div className="text-center mt-20 text-white"></div>
			<div
				id="favorite"
				className="flex flex-col justify-center items-center">
				<h2 className="text-bold text-2xl text-orange-600 ml-8">
					Favorites
				</h2>
